...
class sequence_model(object):
    def __init__(self,
        ...
        ],lr=self.info['TRAIN']['LEARNING_RATE'])
    ...
    def train(self):
        ...
        criterion2 = torch.nn.L1Loss(reduce = False).cuda()
        # criterion2 = torch.nn.CrossEntropyLoss(reduce = False).cuda()
        # criterion2 = FocalLossV1().cuda()
        # criterion1 = torch.nn.MSELoss(size_average=True).cuda()
        # criterion2 = torch.nn.CrossEntropyLoss(size_average=True).cuda()
        step = 0
        for train_iter in range(self.info['TRAIN']['EPOCHES']):
            ...
            for i_batch, batch_data in enumerate(self.train_loader):
                ...
                w_pred = get_reg_weights(self.encoder_decoder_model.models[1], hidden, target_frame_dat)
                w_warp = get_warp_weights(self.encoder_decoder_model.models[1], hidden, target_frame_dat, in_frame_dat[:, -1])
                w_reg = get_regression_weights(self.encoder_decoder_model.models[1], hidden, target_frame_dat)
...
if __name__ == '__main__':
    ...
    path = 'configs/GCMT_ConvLSTM.yml'
    f = open(path)
    ...
    warp_output_conv_cells = []
    for idx, cell in enumerate(configuration['MODEL_NETS']['WARP_OUTPUT_CONV']):
        if idx == len(configuration['MODEL_NETS']['WARP_OUTPUT_CONV']) - 1:
            warp_output_conv_cells.append(
                get_conv_param(cell, padding=configuration['MODEL_NETS']['WARP_OUTPUT_PADDING'][idx], activate='tanh'))
        else:
            warp_output_conv_cells.append(
                get_conv_param(cell, padding=configuration['MODEL_NETS']['WARP_OUTPUT_PADDING'][idx]))
    ...
    for idx, cell in enumerate(configuration['MODEL_NETS']['REG_OUTPUT_CONV']):
        if idx == len(configuration['MODEL_NETS']['REG_OUTPUT_CONV']) - 1:
            ...
            reg_output_conv_cells.append(get_conv_param(cell, padding=configuration['MODEL_NETS']['REG_OUTPUT_PADDING'][idx]))
    
    encoder = Encoder_ConvLSTM(
        conv_rnn_cells = encode_conv_rnn_cells,
        conv_cells = downsample_cells
    )
    decoder = Decoder_ConvLSTM(
        conv_rnn_cells = decode_conv_rnn_cells,
        conv_cells = upsample_cells,
        output_cells = output_conv_cells,
        warp_output_cells = warp_output_conv_cells,
        reg_output_cells = reg_output_conv_cells
    )
    encoder_decoder_model = Encode_Decode_ConvLSTM(
        encoder = encoder,
        decoder = decoder,
        info = configuration
    )
