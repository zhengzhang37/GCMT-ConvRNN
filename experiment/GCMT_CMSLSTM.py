...
def padding_CIKM_data(frame_data):
    ...
    batch_size = shape[0]
    seq_length = shape[1]
    padding_frame_dat = torch.zeros((batch_size,seq_length,1,128,128))
    ...
    def train(self):
        ...
        for train_iter in range(self.info['TRAIN']['EPOCHES']):
            ...
            for i_batch, batch_data in enumerate(self.train_loader):
                frame_dat = batch_data[0]
                in_frame_dat = padding_CIKM_data(frame_dat[:,:5]).cuda()
                target_frame_dat = padding_CIKM_data(frame_dat[:,5:]).cuda()
                target_seg_label = padding_CIKM_data(seg_label[:,5:].unsqueeze(2)).cuda()
                ...
                c_warp = torch.abs(compute_rank_correlation(torch.reshape(w_warp, (1*10, 64*64*64)), torch.reshape(w_pred, (4*10, 64*51*51))))
                c_reg = torch.abs(compute_rank_correlation(torch.reshape(w_reg, (1*10, 64*64*64)), torch.reshape(w_pred, (4*10, 64*51*51))))
                c1 = c_warp / (c_reg + c_warp)
                c2 = c_reg / (c_reg + c_warp)
                c1 = torch.reshape(c1, (1, 10, 1, 1, 1))
                c2 = torch.reshape(c2, (1, 10, 1, 1, 1))
    ...
    def validation(self):
        ...
        with torch.no_grad():
            for i_batch, batch_data in enumerate(self.valid_loader):
                ...
                target_frame_dat = frame_dat[:, 5:].cuda()
                padding_frame_dat = padding_CIKM_data(in_frame_dat).cuda()
                output_frames,_,_,_ = self.encoder_decoder_model(padding_frame_dat)
                output_frames = denormalization(output_frames[:,:,:,13:-14,13:-14].data.cpu().numpy(), 255.0).astype(np.float32) / 255.0
...
if __name__ == '__main__':
    ...
    encoder_decoder_model = Encode_Decode_CMSLSTM(
        encoder = encoder,
        decoder = decoder,
        info = configuration
    )
