...
class SpatioTemporalLSTMCell(nn.Module):
    ...
    def init_state(self, hidden):
        return torch.zeros_like(hidden,dtype=torch.float32).cuda()

    def forward(self, x_t, h_t=None, c_t=None, m_t=None):
        x_concat = self.conv_x(x_t)
        if h_t is None:
            h_t = self.init_state(x_t)
            c_t = self.init_state(x_t)
            m_t = self.init_state(x_t)
